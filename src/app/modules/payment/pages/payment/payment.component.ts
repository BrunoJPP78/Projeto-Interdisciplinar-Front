...
export class PaymentComponent {
  displayedColumns: string[] = ["nome_funcionario", "profissao_funcionario", "salario_funcionario", "salario_descontado", "acoes"];
  dataSource: Usuario[] = [];
  pagamentos: any[] = [];
  ...
  ngOnInit() {
    ...
    if (userDataString !== null) {
      ...
      if (userData && userData.unidade_padrao && userData.unidade_padrao.id) {
        ...
        this.pagamentoService.listUsuarios(unidadeId).subscribe(
          (data: any) => {
  ...
  deleteItem(id: number) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.pagamentoService.deleteUsuario(userId).subscribe(
          () => {
            this.pagamentos = this.pagamentos.filter((data) => data.id !== userId);
